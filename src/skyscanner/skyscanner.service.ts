...
export class SkyscannerService {
  ...
  private getHeaders() {
    ...
    const apiHost = this.configService.get<string>('SKYSCANNER_API_HOST') || 'skyscanner-api.p.rapidapi.com';

    if (!apiKey) {
      throw new HttpException('Skyscanner API key not configured', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  async searchFlights(params: { fromId: string; toId: string; departDate: string }) {
    const baseUrl = this.configService.get<string>('SKYSCANNER_BASE_URL') || 'https://skyscanner-api.p.rapidapi.com/v3';
    console.log(`[Skyscanner] Base URL: ${baseUrl}`);
    try {
      console.log(`[Skyscanner] Requesting: ${baseUrl}/flights/live/search/create`);
      const response = await lastValueFrom(
        this.httpService.get(`${baseUrl}/flights/live/search/create`, {
